                       ...
                       failure:(void(^)(int errorCode))failure
{
    ...
    // fill mode
    _cutPanel.cropMode = (AliyunCropCutMode)AliyunImageCropModeAspectFill;
    _cutPanel.rect = CGRectMake(0, 0, 1, 1);
    
    _cutPanel.videoQuality = (AliyunVideoQuality)AliyunMediaQualityVeryHight;
    _cutPanel.encodeMode = (_config.encodeMode == AliyunEncodeModeSoftFFmpeg)?0:1;
    if (_config.videoQuality == AliyunMediaQualityVeryHight) {
        _cutPanel.bitrate = 15*1000*1000; // 15Mbps
        _cutPanel.encodeMode = 1; // Force hardware encoding
    }
