...
///获取相册封面
- (void)getPostImageWithAlbumModel:(AliyunAlbumModel *)model
...
#pragma mark - new api
- (void)saveGifWithAsset:(PHAsset *)asset
                 maxSize:(CGSize)maxSize
              outputPath:(NSString *)path
              completion:(void (^)(NSError *error))completion;

- (void)savePhotoWithAsset:(PHAsset *)asset
                   maxSize:(CGSize)maxSize
                outputPath:(NSString *)path
                completion:(void (^)(NSError *error, UIImage * _Nullable result))completion;
