...
//    NSLog(@"~~~~~destination:%@", destination);
    ...
    [httpClient download:pasterInfo.url
        ...
        NSString *unzipPath = [[pasterInfo directoryPath] stringByAppendingPathComponent:unzipName];
        NSString *unzipEncodedPath = [unzipPath stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]];
//        NSString *unzipPath = [[[pasterInfo directoryPath] stringByAppendingPathComponent:unzipName] stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            [self upzip:filePath to:[NSURL URLWithString:unzipEncodedPath]];
        });
        if(error){
...
{
    BOOL finish = [SSZipArchive unzipFileAtPath:filePath.path toDestination:toDestination.path];
