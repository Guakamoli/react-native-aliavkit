    ...
    subscription = managerEmitter.addListener(
      ...
      (reminder) => {
        ...
        if (reminder.progress == 1 && this.state.fileSelectType === 'video') {
          ...
          this.props.navigation.push('PostEditorBox', { trimVideoData, videoduration: videoTime, trimmerRight: trimmerRightHandlePosition, fileType: this.state.fileSelectType })
    ...
    return (
      <View style={{ height: 44, backgroundColor: '#000', flexDirection: "row", justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 12 }}>
        <TouchableOpacity onPress={async () => {
          ...
          // 取消按钮
          fileEditor
            ? this.setState({ fileEditor: false, multipleData: [], selectBottomModel: '滤镜', startmMltiple: false, multipleSandBoxData: [], coverImage: "", coverList: [] })
                ...
                <TouchableOpacity onPress={async () => {
                  ...
                  if (multipleData.length <= 1) {
                    ...
                    if (fileType === 'video') {
                      ...
                      // 视频设置编辑器  总数 右边值 同步
                      this.setState({
                        trimmerRightHandlePosition: Math.ceil(item.image.playableDuration) < 300 ? Math.ceil(item.image.playableDuration) * 1000 : 300 * 1000,
...

                    {
                      startmMltiple ? (
                        <>
                          ...
                          >
                            {multipleData.includes(item) ? <View
                              style={{
                                width: 18, height: 18, borderRadius: 20, zIndex: 99, backgroundColor: '#836BFF', justifyContent: 'center', alignItems: 'center'
                              }}
                            >
