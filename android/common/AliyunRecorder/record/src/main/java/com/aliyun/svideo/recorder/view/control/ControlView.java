...
//    private View mAlivcTakePhoto;
    ...
    public void onResume() {
        ...
        if (mScreenOrientationListener != null) {
            mScreenOrientationListener.onResume();
        }
    }

    public void onPause() {
        if (mScreenOrientationListener != null) {
            mScreenOrientationListener.onPause();
        }
    }
    ...
    private void initTopBlessingView() {
        ...
        int black_alpha_color_int = ContextCompat.getColor(getContext(), R.color.alivc_common_bg_black_alpha_10);
        int black_color_int = ContextCompat.getColor(getContext(), R.color.alivc_common_bg_black_alpha_35);
        int white_alpha_color_int = ContextCompat.getColor(getContext(), R.color.alivc_common_bg_white_alpha_20);
        int white_color_int = ContextCompat.getColor(getContext(), R.color.white);

        int screenHeight = ScreenUtils.getHeight(getContext());
        ViewGroup.LayoutParams layoutParams = llVerticalTitleLayout.getLayoutParams();
        layoutParams.height = screenHeight * 9 / 20;
        llVerticalTitleLayout.setLayoutParams(layoutParams);

        tvContentLayout.setBackgroundColor(black_alpha_color_int);
        tvTitle.setTextColor(white_alpha_color_int);
        tvContent.setTextColor(white_alpha_color_int);
        setOnTitleViewUpdateCallback(new ControlView.OnTitleViewUpdateCallback() {
            @Override
            public void onUpdate(RecordState recordState) {
                if (recordState == RecordState.STOP) {
                    ...
                    //深色
                    tvContentLayout.setBackgroundColor(black_color_int);
                    tvTitle.setTextColor(white_color_int);
                    tvContent.setTextColor(white_color_int);
                }
            }
        });
    }
...
//        });
    ...
    private OnTitleViewUpdateCallback mUpdateCallback;


    public void setOnTitleViewUpdateCallback(OnTitleViewUpdateCallback callback) {
        this.mUpdateCallback = callback;
    }

    public interface OnTitleViewUpdateCallback {
        void onUpdate(RecordState recordState);
    }
    ...
    private void updateTittleView() {
        if (mUpdateCallback != null) {
            mUpdateCallback.onUpdate(recordState);
        }
        if (recordState == RecordState.STOP) {
...
//            mTvAspectRatio.setTextColor(ContextCompat.getColor(getContext(), R.color.alivc_common_font_white));
    ...
    private void initRotateSensor() {
        mScreenOrientationListener = new ScreenOrientationListener(getContext(), 0, aliyunRecordBtn);
        List<View> views = new ArrayList<>();

        views.add(aliyunSwitchLight);
        views.add(ivReadyRecord);
        views.add(aliyunSwitchCamera);
//        views.add(aliyunComplete);

        views.add(mAlivcMusic);
        views.add(mLlFilterEffect);
        views.add(mAlivcAnimFilter);

//        views.add(aliyunRateQuarter);
//        views.add(aliyunRateHalf);
//        views.add(aliyunRateOrigin);
//        views.add(aliyunRateDouble);
//        views.add(aliyunRateDoublePower2);

        views.add(llBeautyFace);
        views.add(llGifEffect);

        mScreenOrientationListener.setAngleViews(views);
        mScreenOrientationListener.setBackView(aliyunBack);
        mScreenOrientationListener.setContentView(llVerticalTitleLayout, tvTitle, tvContent);

        if (mScreenOrientationListener.canDetectOrientation()) {
