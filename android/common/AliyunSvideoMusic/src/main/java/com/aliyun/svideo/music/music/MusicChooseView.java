            ...
            mMusicAdapter.setOnMusicSeekListener(new MusicAdapter.OnMusicSeek() {
                ...
                public void onShowMusicVolume() {
                    if (musicSelectListener != null) {
                        musicSelectListener.onShowMusicVolume();
                    }
                }
...
//        mAlivcMusicCopyrightTV.setVisibility(View.INVISIBLE);
    ...
    public void reSetSelectedPosition() {
        if(mAliyunMusicRecyclerView!=null){
            mAliyunMusicRecyclerView.scrollToPosition(0);
        }
        if (mMusicAdapter != null) {
            mMusicAdapter.reSetSelectedPosition();
            if (mAliyunOnlineMusic != null && mAliyunLocalMusic != null) {
                if (mAliyunOnlineMusic.isSelected() && mOnlineMusicList != null && !mOnlineMusicList.isEmpty()) {
                    mMusicAdapter.setData(mOnlineMusicList, 0);
                } else if (mAliyunLocalMusic.isSelected() && mLocalMusicList != null && !mLocalMusicList.isEmpty()) {
                    mMusicAdapter.setData(mLocalMusicList, 0);
                }
            }
        }
