...
public class AliyunVideoCropActivity extends Activity implements TextureView.SurfaceTextureListener, HorizontalListView.OnScrollCallBack, SizeChangedNotifier.Listener,
    ...
    private void getData() {
        ...
        try {
            videoDuration = crop.getVideoDuration(mInputVideoPath) / 1000;
        } catch (Exception e) {
//            ToastUtil.showToast(getApplicationContext(), R.string.alivc_crop_video_tip_crop_failed);
        }
        duration  = intent.getIntExtra("aliyun_mediaInfo_duration", (int) videoDuration);
        //获取精确的视频时间
        mBitrate = intent.getIntExtra(AlivcCropInputParam.INTENT_KEY_BITRATE, AlivcCropInputParam.DEFAULT_VALUE_BITRATE);
    ...
    private void initView() {
        ...
        dirationTxt.setText(convertDuration2Text(duration));
    ...
    private String convertDuration2Text(long duration) {
        ...
        int min = (sec % 3600) / 60;
        sec = (sec % 60);
        return String.format(getString(R.string.alivc_media_video_duration_mmmss),
                hour,
                min,
                sec);
    }
            ...
            mPlayer.setPlayerCallback(new PlayerCallback() {
                ...
                public void onDataSize(int dataWidth, int dataHeight) {
                    ...
                    if (crop != null && mEndTime == 0) {
                        try {
                            ...
                            ToastUtil.showToast(getApplicationContext(), R.string.alivc_crop_video_tip_error);
                        }
                    }
                ...
                public void onError(int i) {
                    ...
                    ThreadUtils.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtil.showToast(getApplicationContext(), getString(R.string.alivc_crop_video_tip_error));
                        }
                    });
    ...
    private VideoSliceSeekBar.SeekBarChangeListener mSeekBarListener = new VideoSliceSeekBar.SeekBarChangeListener() {
        ...
        public void seekBarValueChanged(float leftThumb, float rightThumb, int whitchSide) {
            ...
            dirationTxt.setText(convertDuration2Text(mEndTime - mStartTime));
    ...
    private void startCrop() {
        ...
        if (!PermissionUtils.checkPermissionsGroup(this, new String[]{Manifest.permission.READ_EXTERNAL_STORAGE})) {
            ToastUtils.show(getApplicationContext(), PermissionUtils.NO_PERMISSION_TIP[4]);
            return;
        }
        ...
        if (ret < 0) {
            ToastUtil.showToast(getApplicationContext(), getString(R.string.alivc_crop_video_tip_crop_failed) + "  " + ret);
            return;
        }
            ...
            public void run() {
                ...
                switch (code) {
                    case AliyunErrorCode.ALIVC_SVIDEO_ERROR_MEDIA_NOT_SUPPORTED_VIDEO:
                        ToastUtil.showToast(getApplicationContext(), R.string.alivc_crop_video_tip_not_supported_video);
                        break;
...
//                progressDialog.dismiss();
    ...
    public static void startVideoCropForResult(Activity context, AlivcCropInputParam param, int requestCode) {
        ...
        intent.putExtra(AlivcCropInputParam.INTENT_KEY_BITRATE, param.getBitrate());
        ...
        intent.putExtra("aliyun_mediaInfo_duration", param.getMediaInfo().duration);
