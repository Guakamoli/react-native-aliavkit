...
import com.aliyun.svideo.editor.publish.paiya.PaiyaPublishActivity;
 ...
 * 拍鸭：视频编辑
 */
public class EditorActivity extends BaseAliActivity {
    private static final String TAG = "EditorActivity";
    private static final String NEXT_ACTIVITY_CLASS_NAME = "com.aliyun.svideo.editor.publish.paiya.PublishActivity2";
...
//    private String mCopyUriPath;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editView.setBlessingData(mStrTitle,mStrContent);
        ...
        mMediaInfoList = mInputParam.getMediaInfos();

        /**
         * 转换JSON文件
         */
        Uri mUri = Uri.fromFile(new File(getProjectJsonPath(mMediaInfoList)));
//        File file = new File(mUri.getPath());
//        String parentPath = file.getParent();
//        mCopyUriPath = parentPath + "/project_copy.json";

        editView.setHasDeNoise(isDeNoise);
        editView.setParam(mVideoParam, mUri, hasTailAnimation, mInputParam.isHasWaterMark(),mMediaInfoList);
        editView.setReplaceMusic(isReplaceMusic);
        editView.setHasRecordMusic(isReplaceMusic);
        editView.setIsMixRecord(isMixRecorder);
        editView.setShowTrim(isShowTrim);
        editView.setmOnFinishListener(new AlivcEditView.OnFinishListener() {
            @Override
            public void onComplete(AlivcEditOutputParam outputParam, AliyunIEditor mAliyunIEditor) {
//                boolean isCopy = com.blankj.utilcode.util.FileUtils.copy(mUri.getPath(), mCopyUriPath);
//                Uri uri = EditorVideHelper.setVideoTimes(getApplicationContext(),mVideoParam,mAliyunIEditor,outputParam.getVideoStartTime(),outputParam.getVideoEndTime());
                //编辑完成跳转到其他界面
                Intent intent = new Intent(EditorActivity.this, PaiyaPublishActivity.class);
//                intent.setClassName();
                intent.putExtra(PaiyaPublishActivity.KEY_PARAM_THUMBNAIL, outputParam.getThumbnailPath());
//                intent.putExtra(PublishActivity2.KEY_PARAM_CONFIG, uri.getPath());
                intent.putExtra(PaiyaPublishActivity.KEY_PARAM_CONFIG, outputParam.getConfigPath());
                intent.putExtra(PaiyaPublishActivity.KEY_PARAM_VIDEO_WIDTH, outputParam.getOutputVideoWidth());
                intent.putExtra(PaiyaPublishActivity.KEY_PARAM_VIDEO_HEIGHT, outputParam.getOutputVideoHeight());
                //传入视频比列
                intent.putExtra(PaiyaPublishActivity.KEY_PARAM_VIDEO_RATIO, outputParam.getVideoRatio());
    ...
    private void initData() {
        ...
        if (isMixRecord) {
            ...
            ArrayList<MediaInfo> mediaList = mInputParam.getMediaInfos();
            if (mediaList != null && !mediaList.isEmpty()) {
                int videoWidth = mVideoParam.getOutputWidth();
                int videoHeight = mVideoParam.getOutputHeight();
                ...
                try {
                    String filePath = mediaList.get(0).filePath;
                    MediaMetadataRetriever retriever = new MediaMetadataRetriever();
                    retriever.setDataSource(filePath);
                    rotation = Integer.parseInt(retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_ROTATION));

                    int widthSrcWidth = Integer.parseInt(retriever.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH));
                    int widthSrcHeight = Integer.parseInt(retriever.extractMetadata(android.media.MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT));


//                    Log.e("AAA", "videoWidth: " + widthSrcWidth + "; videoHeight: " + widthSrcHeight + "; rotation: " + rotation);

//                    if ((rotation == 0 || rotation == 180) && widthSrcWidth > widthSrcHeight) {
//                        rotation = 360 - rotation - 90;
//                    }

                    if ( widthSrcWidth > widthSrcHeight) {
                        rotation = 360 - rotation - 90;
                    }
                ...
                if (rotation == 90 || rotation == 270) {
                    mVideoParam.setOutputHeight(videoWidth);
                    mVideoParam.setOutputWidth(videoHeight);
                }
                editView.setVideoRotation(rotation);
    ...
    protected void onStop() {
        ...
        if (isFinishing()) {
            if (mMediaInfoList != null && !mMediaInfoList.isEmpty()) {
                for (MediaInfo info : mMediaInfoList) {
                    FileUtils.deleteFileOnExit(info.filePath);
                }
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        Log.e("CCC", "onActivityResult requestCode：" + requestCode + "；resultCode：" + resultCode);
        if (editView != null) {
    ...
    public static void startEdit(Context context, AlivcEditInputParam param, String strTitle, String strContent,boolean  isShowTrim) {
