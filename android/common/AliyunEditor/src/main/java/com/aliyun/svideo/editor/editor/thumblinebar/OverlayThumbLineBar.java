...
public class OverlayThumbLineBar extends ThumbLineBar {
    ...
    @Override
    public void setup(ThumbLineConfig thumbLineConfig, OnBarSeekListener barSeekListener, AlivcEditView.PlayerListener linePlayer,long mStartTime,long  mEndTime) {
        super.setup(thumbLineConfig, barSeekListener, linePlayer,mStartTime, mEndTime);
    }
    ...
    public void showOverlay(UIEditorPage uiEditorPage) {
        ...
        boolean isCaption;
        isCaption = uiEditorPage == UIEditorPage.FONT || uiEditorPage == UIEditorPage.TEXT;

        for (ThumbLineOverlay overlay : mOverlayList) {

            if (uiEditorPage == overlay.getUIEditorPage()) {
                overlay.getOverlayView().setVisibility(VISIBLE);
            } else if ((isCaption && (overlay.getUIEditorPage() == UIEditorPage.TEXT || overlay.getUIEditorPage() == UIEditorPage.FONT))) {
                //如果是字幕，将字体和字幕全部显示
                overlay.getOverlayView().setVisibility(VISIBLE);
            } else {
