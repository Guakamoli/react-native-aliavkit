                ...
                if (tokenInfo != null && mComposeClient != null) {
                    ...
                    if (rv < 0) {
                        ...
                        ThreadUtils.runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                ToastUtil.showToast(getApplicationContext(), getResources().getString(R.string.alivc_editor_publish_upload_param_error));
                            }
                        });
                    } else {
                        mIsUpload = true;
                    ...
                    ThreadUtils.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtil.showToast(getApplicationContext(), "Get image upload auth info failed");
                        }
                    });
            ...
            protected void onSuccess(String s) {
                ...
                if (tokenInfo != null && mComposeClient != null) {
                    ...
                    ThreadUtils.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            ToastUtil.showToast(getApplicationContext(), "Get video upload auth failed");
                        }
                    });
