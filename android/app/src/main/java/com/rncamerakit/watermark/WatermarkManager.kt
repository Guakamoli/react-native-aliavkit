...
//                bitmap = textToImage(context, watermarkText)
//                val path = bitmapToPath(context, bitmap)
//                val effectPicture: EffectPicture = getPathWaterMark(context, videoParam, path)
//                mAliyunIEditor?.addImage(effectPicture)
            }
...
//                    }
        ...
        private fun getPathWaterMark(context: Context, videoParam: VideoParamBean, imagePath: String): EffectPicture {
             ...
             * 水印宽高
             */
            videoParam.watermarkWidth = imageWidth.toFloat()/videoParam.outputWidth*scale
            videoParam.watermarkHeight = imageHeight.toFloat()/videoParam.outputHeight*scale
        ...
        private fun textToImage(context: Context, text: String?): Bitmap {
            val watermarkLogoWidth = DensityUtils.dip2px(context, 87F).toFloat()
            val watermarkLogoHeight = DensityUtils.dip2px(context, 40F).toFloat()
            var text = text
            if (text == null) {
                text = ""
            }
            ...
            val bitmap = Bitmap.createBitmap(bitmapWidth.toInt(), (watermarkLogoHeight + intervalWidth + textSize).toInt(), Bitmap.Config.ARGB_8888)
            ...
            canvas.drawText(text, textStartX, watermarkLogoHeight + textSize - intervalWidth, mTextPaint)
            // 保存绘图为本地图片
            canvas.save()
            canvas.restore()
        ...
        private fun bitmapToPath(context: Context, bitmap: Bitmap): String {
